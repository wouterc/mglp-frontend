...
function Sagsoversigt() {
  ...
  const [alleStatusser, setAlleStatusser] = useState([]);
  ...
  async function hentData() {
    try {
      const [sagerRes, statusRes] = await Promise.all([
        fetch('http://127.0.0.1:8000/api/sager/'),
      ...
      setSager(sagerData);
      setAlleStatusser(statusData);
    } catch (e) {
      setError('Kunne ikke hente data. Sikr at backend-serveren kører.');
  ...
  const sorteredeOgFiltreredeSager = useMemo(() => {
    ...
    // NY AVANCERET FILTRERINGSLOGIK
    const isSpecificSearch = filter.sags_nr || filter.status;

    if (isSpecificSearch) {
      // Hvis der søges specifikt, ignorer ALLE andre filtre (inkl. checkboxes og andre tekstfelter)
      resultat = sager.filter(sag => {
        if (filter.sags_nr) {
            return sag.sags_nr?.toString().includes(filter.sags_nr);
        }
        if (filter.status) {
      ...
      // Ellers, anvend den normale kombination af filtre
      resultat = resultat.filter(sag => {
        if (!sag.status) return !visLukkede && !visAnnullerede;
        const kategori = sag.status.status_kategori;
        if (!visLukkede && !visAnnullerede) return kategori === 0;
        if (visLukkede && kategori === 1) return true;
        if (visAnnullerede && kategori === 9) return true;
        return false;
      });

      if (filter.alias) {
        resultat = resultat.filter(sag => sag.alias?.toLowerCase().includes(filter.alias.toLowerCase()));
      }
      if (filter.hovedansvarlige) {
        resultat = resultat.filter(sag => sag.hovedansvarlige?.toLowerCase().includes(filter.hovedansvarlige.toLowerCase()));
      }
      if (filter.adresse) {
        resultat = resultat.filter(sag => sag.fuld_adresse?.toLowerCase().includes(filter.adresse.toLowerCase()));
      }
    }
  ...
  const handleOpretNySag = () => { setSagTilRedigering(null); setVisForm(true); }; const handleRedigerSag = (sag) => { setSagTilRedigering(sag); setVisForm(true); }; const handleAnnuller = () => { setVisForm(false); setSagTilRedigering(null); }; const handleRaekkeKlik = (sagId) => { setUdfoldetSagId(udfoldetSagId === sagId ? null : sagId); };
  const handleSaveSag = () => {
    hentData();
    setVisForm(false);
  ...
  if (visForm) return <SagsForm onSave={handleSaveSag} onCancel={handleAnnuller} sagTilRedigering={sagTilRedigering} alleStatusser={alleStatusser} />;
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      {visFlereFiltre && (<div className="mt-4 pt-4 border-t border-gray-200"><div className="flex flex-wrap items-center gap-4"><label className="flex items-center space-x-2 cursor-pointer"><input type="checkbox" checked={visLukkede} onChange={() => setVisLukkede(!visLukkede)} className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"/><span>Vis lukkede</span></label><label className="flex items-center space-x-2 cursor-pointer"><input type="checkbox" checked={visAnnullerede} onChange={() => setVisAnnullerede(!visAnnullerede)} className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"/><span>Vis annullerede</span></label></div></div>)}</div>
        ...
        <table className="min-w-full bg-white">
          <thead className="bg-gray-800 text-white">
            <tr>
              ...
              <th className="text-left py-2 px-4 uppercase font-semibold text-sm cursor-pointer" onClick={() => requestSort('status.beskrivelse')}>Status{getSortIcon('status.beskrivelse')}</th>
