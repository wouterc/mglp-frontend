...
function KontaktForm({ onSave, onCancel, kontaktTilRedigering, defaultRolleIds = [], onSaveNyKontakt }: KontaktFormProps): ReactElement {
  ...
  useEffect(() => {
    const fetchDataForForm = async () => {
        setIsLoading(true);
        try {
            const [virksomhederRes, rollerRes] = await Promise.all([
                fetch(`${API_BASE_URL}/register/virksomheder/?limit=1000`),
                fetch(`${API_BASE_URL}/register/roller/`)
            ]);
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const url = erRedigering
      ? `${API_BASE_URL}/register/kontakter/${kontaktTilRedigering?.id}/`
      : `${API_BASE_URL}/register/kontakter/`;
    ...
    try {
      ...
      if (!erRedigering && onSaveNyKontakt) {
        const nyKontakt = await response.json();
