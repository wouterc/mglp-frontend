...
import { Edit, ArrowUp, ArrowDown } from 'lucide-react'; // Importer ikoner
...
function Sagsoversigt() {
  ...
  // Ny state til at håndtere sortering
  ...
  // useMemo bruges til at genberegne den filtrerede og sorterede liste,
  // kun når de relevante data (sager, filter, sortering) ændrer sig.
  const sorteredeOgFiltreredeSager = useMemo(() => {
    let filtreret = [...sager];

    // Status Filter
    filtreret = filtreret.filter(sag => {
        const statusValue = sag.status;
        if (statusValue === null || statusValue < 90) return true; 
        if (visLukkede && statusValue === 90) return true;
        if (visAnnullerede && statusValue === 99) return true;
        return false;
    });

    // Tekst-filtre
    if (filter.sags_nr) filtreret = filtreret.filter(s => s.sags_nr?.toString().includes(filter.sags_nr));
    if (filter.status) filtreret = filtreret.filter(s => s.status?.toString().includes(filter.status));
    if (filter.alias) filtreret = filtreret.filter(s => s.alias?.toLowerCase().includes(filter.alias.toLowerCase()));
    if (filter.hovedansvarlige) filtreret = filtreret.filter(s => s.hovedansvarlige?.toLowerCase().includes(filter.hovedansvarlige.toLowerCase()));
    if (filter.adresse) filtreret = filtreret.filter(s => s.fuld_adresse?.toLowerCase().includes(filter.adresse.toLowerCase()));
    
    // Sortering
    if (sortConfig.key !== null) {
      filtreret.sort((a, b) => {
        // Håndter null-værdier for at undgå fejl
  ...
  const handleNulstilFiltre = () => {
    setFilter({ sags_nr: '', alias: '', hovedansvarlige: '', adresse: '', status: '' });
    setVisLukkede(false);
    setVisAnnullerede(false);
  };
  ...
  const handleSaveSag = async (sagsData) => {
    const erRedigering = sagsData.id != null;
    const url = erRedigering ? `http://127.0.0.1:8000/api/sager/${sagsData.id}/` : 'http://127.0.0.1:8000/api/sager/';
    const method = erRedigering ? 'PUT' : 'POST';
    try {
      const response = await fetch(url, { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(sagsData) });
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="flex justify-between items-center mb-4">
        ...
        <button onClick={handleOpretNySag} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">Opret Ny Sag</button>
      </div>
      
      <div className="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-4">
          <input type="text" name="sags_nr" placeholder="Sagsnr..." value={filter.sags_nr} onChange={handleFilterChange} className="p-2 border border-gray-300 rounded-md lg:col-span-2"/>
          <input type="text" name="status" placeholder="Status..." value={filter.status} onChange={handleFilterChange} className="p-2 border border-gray-300 rounded-md lg:col-span-1"/>
          <input type="text" name="alias" placeholder="Alias..." value={filter.alias} onChange={handleFilterChange} className="p-2 border border-gray-300 rounded-md lg:col-span-3"/>
          <input type="text" name="hovedansvarlige" placeholder="Ansvarlig..." value={filter.hovedansvarlige} onChange={handleFilterChange} className="p-2 border border-gray-300 rounded-md lg:col-span-3"/>
          <input type="text" name="adresse" placeholder="Adresse..." value={filter.adresse} onChange={handleFilterChange} className="p-2 border border-gray-300 rounded-md lg:col-span-3"/>
        </div>
        ...
        <div className="flex justify-between items-center mt-4">
          ...
          <button onClick={handleNulstilFiltre} className="px-4 py-2 bg-gray-600 text-white rounded-md hover:bg-gray-700 text-sm">Nulstil Alle Filtre</button>
        </div>
          ...
          <div className="mt-4 pt-4 border-t border-gray-200">
            <div className="flex flex-wrap items-center gap-4">
              ...
              <label className="flex items-center space-x-2 cursor-pointer">
                ...
                <span>Inkluder annullerede (99)</span>
              </label>
            </div>
          </div>
        )}
              ...
              <React.Fragment key={sag.id}>
                <tr onClick={() => handleRaekkeKlik(sag.id)} className="border-b border-gray-200 hover:bg-gray-100 cursor-pointer">
                  ...
                  <td className="py-2 px-4">
                    <button onClick={(e) => { e.stopPropagation(); handleRedigerSag(sag); }} className="text-gray-500 hover:text-blue-600">
                      <Edit size={18} />
                    </button>
                  </td>
                </tr>
