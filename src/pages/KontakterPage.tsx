...
import { PlusCircle, AlertCircle, Edit, Loader2, X, Copy, Check, MessageSquare } from 'lucide-react';
...
function KontakterPage(): ReactElement {
    ...
    const [filterVirksomhed, setFilterVirksomhed] = useState('');
    const [filterTelefon, setFilterTelefon] = useState('');
    const [filterEmail, setFilterEmail] = useState('');
    
    const debouncedNavn = useDebounce(filterNavn, 300);
    const debouncedRolle = useDebounce(filterRolle, 300);
    const debouncedVirksomhed = useDebounce(filterVirksomhed, 300);
    const debouncedTelefon = useDebounce(filterTelefon, 300);
    const debouncedEmail = useDebounce(filterEmail, 300);
    ...
    const hentKontakter = useCallback(async () => {
        ...
        try {
            ...
            dispatch({ type: 'SET_KONTAKTER_STATE', payload: { kontakter: kontakterListe || [], erKontakterHentet: true } });
             ...
             dispatch({ type: 
'SET_KONTAKTER_STATE', payload: { kontakterError: e.message } });
        } finally {
    ...
    const handleCopyAddress = (kontakt: Kontakt, e: MouseEvent) => {
        ...
        const addressString = [
            kontakt.adresse_vej,
            `${kontakt.adresse_postnr ||
''} ${kontakt.adresse_by || ''}`.trim()
        ].filter(Boolean).join('\n');
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8">
            ...
            {/* @# 2025-11-08 14:02 - Opdateret filterblok til 5 felter */}
            <div className="mb-4 py-4 px-2 bg-gray-50 rounded-lg border border-gray-200 flex">
                
  
                {/* @# 2025-11-08 14:02 - Justeret bredde 25% */}
                <div className="relative w-[25%] pr-4">
                    <input 
                        ...
                        value={filterNavn}
                        onChange={(e: ChangeEvent<HTMLInputElement>) => setFilterNavn(e.target.value)}
                        className="w-full p-2 border rounded-md text-sm pr-7"
      
                    />
                    {filterNavn && (
                        <button onClick={() => setFilterNavn('')} className="absolute right-6 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600" title="Ryd felt">
                            <X 
size={16} />
                        </button>
                    )}
                ...
                {/* @# 2025-11-08 14:02 - Justeret bredde 10% */}
               
                 <div className="relative w-[10%] pr-4">
                    <select
                        value={filterRolle}
                        onChange={(e: ChangeEvent<HTMLSelectElement>) => setFilterRolle(e.target.value)}
                        className="w-full 
p-2 border rounded-md text-sm bg-white"
                    >
                ...
                {/* @# 2025-11-08 14:02 - Justeret bredde 20% */}
                ...
                <div className="relative w-[20%] pr-4">
                    <input 
                        ...
                        value={filterVirksomhed}
                        onChange={(e: ChangeEvent<HTMLInputElement>) => setFilterVirksomhed(e.target.value)}
                        className="w-full p-2 border rounded-md text-sm pr-7"
                    />
                  
                    {filterVirksomhed && (
                        <button onClick={() => setFilterVirksomhed('')} className="absolute right-6 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600" title="Ryd felt">
                            <X size={16} />
                        </button>
                ...
                <div className="relative w-[10%] pr-4">
                    ...
                    <input 
                        ...
                        onChange={(e: ChangeEvent<HTMLInputElement>) => setFilterTelefon(e.target.value)}
                        className="w-full p-2 border rounded-md text-sm pr-7"
                    />
                    {filterTelefon && (
                 
                        <button onClick={() => setFilterTelefon('')} className="absolute right-6 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600" title="Ryd felt">
                            <X size={16} />
                        </button>
                    )}
                ...
                {/* @# 2025-11-08 14:02 - Justeret bredde 35% */}
                <div className="relative w-[35%]">
                    <input 
                        ...
                        value={filterEmail}
                        onChange={(e: ChangeEvent<HTMLInputElement>) => setFilterEmail(e.target.value)}
                        className="w-full p-2 
border rounded-md text-sm pr-7"
                    />
                    {filterEmail && (
                        <button onClick={() => setFilterEmail('')} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600" title="Ryd felt">
            ...
            <div className="overflow-x-auto rounded-lg shadow-md">
                ...
                <table className="min-w-full bg-white">
                    <thead className="bg-gray-800 text-white text-sm">
                        <tr>
                            {/* @# 2025-11-08 14:02 - Opdateret kolonneoverskrifter og bredder */}
                            ...
                            <th className="text-left py-1 px-2 uppercase 
font-semibold w-[20%]">Virksomhed</th>
...
px-2 uppercase font-semibold w-[50px]"></th>
                        </tr>
                    </thead>
...
'').trim() && (
                                    ...
                                    {/* @# 2025-11-08 14:25 - Opdateret til at vise formateret navn */}
         
                                    <td className="py-1 px-2">{formatVirksomhedsnavn(k.virksomhed)}</td>
...
(
                                                ...
                                                <div>
                                                    ...
                                                    <p className="whitespace-pre-wrap break-words">{k.kommentar ||
<span className="italic text-gray-500">Ingen kommentar</span>}</p>
                                                </div>
                                            </div>
